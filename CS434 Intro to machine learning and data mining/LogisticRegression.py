...
Original file is located at
    https://colab.research.google.com/drive/15SbfCRxM6BN5V8vnBom8kEBUnq8ISaCH
"""
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  nll = -sum(y * np.log(logistic(np.dot(X,w)) +  0.0000001) + (1-y) * np.log(1 - logistic(np.dot(X,w)) +  0.0000001 ))
  #print(nll)
  return nll
...
def trainLogistic(X,y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
        ...
        # .
        w_grad = np.dot(np.transpose(X),(logistic(np.dot(X,w)) - y))
...
def dummyAugment(X):
  ...
  new_X = np.append(np.full((rows, 1), 1), X, axis = 1)
  return new_X
